...
#endif
    using DecoderType = roman_numerals::RomanDecoder<roman_numerals::RomanNumberValidator>;

    auto roman_endecoder = NumberEncoderDecoder<roman_numerals::RomanEncoder, DecoderType>{};
    try {
        if ((not roman_number.empty()) && (arabic_number <= 4000u)) {
            auto const arabic_result = roman_endecoder.decode(roman_number);
